...
function init() {
	...
	GEO_TYPES.forEach(function(type) {
	...
	// objMaterial.roughnessMap = loader.load(scratch);
	// objMaterial.bumpMap = loader.load(scratch);
...
function update(renderer, scene, camera, clock) {
  ...
  var currentIndex = Math.floor((clock.getElapsedTime() / 4) % GEO_TYPES.length);
